...
describe ( 'pgDal', () => {
  it ( "should create tables, insert to the them and read from them", async () => {
    if ( inCi ) return
    try {
      await dal.update ( "drop table if exists driver" )
      ...
      await dal.update ( "create table if not exists driver (id int, name text)" )
      ...
      await dal.update ( "insert into driver (id, name) values ($1, $2)", 1, "phil" )
      await dal.update ( "insert into driver (id, name) values ($1, $2)", 2, "joe" )
      ...
      const res = await dal.query ( "select * from driver" )
